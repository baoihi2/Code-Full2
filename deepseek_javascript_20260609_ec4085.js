...
(function() {
    ...
    class AutoClickThread {
        ...
        start() {
            ...
            if (this.currentIndex === 0 && this.isRunning) {
                setTimeout(() => this.processTrigger(triggers[0]), 1000 * (this.threadId + 1));
            }
        }
    }
    ...
    function updatePanelDisplay() {
        ...
        if (isMenuCollapsed) {
            ...
            for (let i = 0; i < threads.length; i++) {
                const t = threads[i];
                threadsHtml += `
                    <div style="margin: 3px 0; color: ${t.isRunning ? '#69f0ae' : '#ff6b6b'}; display: flex; justify-content: space-between;">
                        <span>🔹 Luồng ${i+1}</span>
                        <span style="font-size: 9px;">${t.userName.substring(0, 8)}...</span>
                        <span>${t.isRunning ? '● RUN' : '● STOP'}</span>
                    </div>
                `;
            }
            ...
            controlPanel.innerHTML = `
                ...
                <div style="font-size: 10px; margin-bottom: 8px; max-height: 150px; overflow-y: auto;">
                    ${threadsHtml}
                </div>
    ...
    // ========== KHỞI TẠO ==========
    function init() {
        console.log("🖥️ Đang khởi tạo giao diện...");
        ...
        threads.forEach(thread => {
            setTimeout(() => thread.start(), thread.threadId * 1000);
        });
        ...
        console.log(`📌 Click − để thu nhỏ bảng, click 🎮 để mở rộng`);
    }

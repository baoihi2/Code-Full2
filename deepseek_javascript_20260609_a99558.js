...
(function() {
    ...
    let pageReady = false;
    let spamInterval = null;  // Interval cho spam click
    let isSpamActive = false;  // Đang spam click
    ...
    function startSpamClick(trigger) {
        ...
        // Click ngay lập tức lần đầu
        clickAt(trigger.x, trigger.y, `${trigger.name} (spam)`);
        
        // Thiết lập interval spam
        spamInterval = setInterval(() => {
            if (!isRunning || !isSpamActive || isProcessing) {
                return;
            }
            // Kiểm tra nếu URL của trigger 2 đã xuất hiện thì dừng spam
            const trigger2 = triggers[1]; // trigger 2 là index 1
    ...
    async function processTrigger(trigger) {
        ...
        // Nếu là trigger spam (con trỏ 1) thì xử lý đặc biệt
        if (trigger.isSpam) {
            console.log(`%c🎯 BẮT ĐẦU SPAM: ${trigger.name}`, "color: #ff9800; font-weight: bold");
            startSpamClick(trigger);
            // Không đánh dấu là đã xử lý xong, vẫn ở trigger 1 cho đến khi URL trigger 2 xuất hiện
            return;
        }
        ...
        // Đánh dấu URL đã xử lý (nếu có URL)
    ...
    async function checkNetworkAndWait() {
        ...
        // Nếu đang spam (con trỏ 1) thì kiểm tra URL của trigger 2
        if (isSpamActive) {
            ...
            if (trigger2 && trigger2.fullUrl) {
                ...
                if (found && !seenUrls.has(trigger2.fullUrl)) {
                    ...
                    // Dừng spam và xử lý trigger 2
                    stopSpamClick();
                    // Đánh dấu trigger 1 đã hoàn thành
                    currentIndex = 1;
                    saveState();
                    processTrigger(trigger2);
                }
        ...
        // Nếu trigger không cần URL (needUrl = false) -> click ngay lập tức
        if (!current.needUrl) {
        ...
        // Nếu cần URL nhưng không có URL -> bỏ qua
        if (!current.url) {
    ...
    async function init() {
        ...
        // Bắt đầu spam ngay nếu đang ở trigger 1

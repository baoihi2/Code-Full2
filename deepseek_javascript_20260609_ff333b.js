...
(function() {
    ...
    function createHiddenTab(userName, tabIndex) {
        // Tạo URL với userName riêng - KHÔNG dùng hidden parameter để tránh lỗi
        const currentUrl = window.location.href;
        let baseUrl = currentUrl.split('?')[0];
        // Xóa các parameter cũ
        baseUrl = baseUrl.split('?')[0];
        const newUrl = baseUrl + `?userName=${userName}&tabId=${tabIndex}`;
    ...
    // ========== TẠO PANEL ĐIỀU KHIỂN CHO TAB CHÍNH (CÓ THU NHỎ) ==========
    ...
    function updatePanelDisplay() {
        ...
        if (isMenuCollapsed) {
            // Chế độ thu nhỏ - chỉ hiện nút mở
            ...
            document.getElementById('collapse-menu-btn').onclick = () => {
    ...
    // ========== THÊM STYLE CHO TAB ẨN ==========
    function applyHiddenTabStyle() {
        if (isHiddenTab()) {
            // Ẩn toàn bộ nội dung trang nhưng vẫn chạy script
            ...
            style.textContent = `
                ...
                /* Vẫn cho phép script chạy nhưng ẩn giao diện */
            `;
